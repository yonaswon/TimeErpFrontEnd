...
interface CreateModificationOverlayProps {
  mockupId: number
  leadId: number
...
export default function CreateModificationOverlay({ 
  mockupId, 
  leadId, 
  previousModification = null, 
  onClose, 
  onSuccess 
}: CreateModificationOverlayProps) {
  return (
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-white dark:bg-zinc-800 border-b border-gray-200 dark:border-zinc-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Create Modification</h1>
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                Add a new modification for mockup #{mockupId}
                {previousModification && ` based on modification #${previousModification}`}
              </p>
            </div>
