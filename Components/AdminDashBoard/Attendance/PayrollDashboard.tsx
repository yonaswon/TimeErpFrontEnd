...
import { Info, Save, CheckCircle2, AlertTriangle, Loader2, Download, Clock, Plus } from 'lucide-react';
import api from '../../../api';
...
import { mergeEditsIntoLogs, calculatePayrollRow, formatSalary, calculateExpectedHoursFromLogs, calculateTotalPenaltyHours, calculateOvertimeAmount, solveBasicFromAgreedNet } from './attendanceUtils';
...
interface PayrollDashboardProps {
    ...
    /** Payroll month for saving OT/loans (e.g. "May 2026") — derived from attendance period end date */
    payrollPeriod: string;
    /** Human label for export title (e.g. "2026-04-06 to 2026-05-06") */
...
export default function PayrollDashboard({
    employees, localEdits, holidays, exemptions, payrollPeriod, exportPeriodLabel,
}: PayrollDashboardProps) {
    const period = payrollPeriod;

    const [config, setConfig] = useState<PayrollConfig>({
        agreedNetPays: {}, workingDays: {}, transports: {}, taxableTransports: {},
        defaultTransport: DEFAULT_TRANSPORT, ignoreTimePenalty: {},
    });
    ...
    const [overtimeTarget, setOvertimeTarget] = useState<string | null>(null);
    ...
    useEffect(() => {
        api.get('/lead/attendance/salary-config/')
            .then((res) => {
                ...
                const agreedNetPays: Record<string, string> = {};
                const transports: Record<string, string> = {};
                configs.forEach((c) => {
                    if (parseFloat(c.base_salary) > 0) agreedNetPays[c.zkt_user_id] = c.base_salary;
                    transports[c.zkt_user_id] = c.transport_allowance;
                });
                setConfig((prev) => {
                    const allTransports = { ...transports };
                    const allWorkingDays: Record<string, string> = {};
                    employees.forEach((emp) => {
                        if (!(emp.zkt_user_id in allTransports)) allTransports[emp.zkt_user_id] = prev.defaultTransport;
                        allWorkingDays[emp.zkt_user_id] = prev.workingDays[emp.zkt_user_id] ?? DEFAULT_WORKING_DAYS;
                    });
                    return { ...prev, agreedNetPays, transports: allTransports, workingDays: allWorkingDays };
                });
            })
            .catch(() => {
                setConfig((prev) => {
                    ...
                    const allWorkingDays: Record<string, string> = {};
                    employees.forEach((emp) => {
                        allTransports[emp.zkt_user_id] = prev.defaultTransport;
                        allWorkingDays[emp.zkt_user_id] = DEFAULT_WORKING_DAYS;
                    });
                    return { ...prev, transports: allTransports, workingDays: allWorkingDays };
                });
            });
    }, [employees]);
    ...
    const persistExtra = useCallback(async (zktId: string, overtimeEntries: OvertimeEntry[], loanAmount: number) => {
        try {
            await api.post('/lead/attendance/payroll-extra/', {
                period, zkt_user_id: zktId, overtime_entries: overtimeEntries, loan_amount: loanAmount,
            });
        } catch (err) {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            const configs = employees.map((emp) => ({
                ...
                base_salary: parseFloat(config.agreedNetPays[emp.zkt_user_id] || '0') || 0,
    ...
    const rows = useMemo(() => {
        return employees.map((emp) => {
            ...
            const agreedNetPay = parseFloat(config.agreedNetPays[emp.zkt_user_id] || '0') || 0;
            const workingDays = parseFloat(config.workingDays[emp.zkt_user_id] || DEFAULT_WORKING_DAYS) || 30;
            const nonTaxableTransport = parseFloat(config.transports[emp.zkt_user_id] ?? config.defaultTransport) || 0;
            const taxableTransport = parseFloat(config.taxableTransports[emp.zkt_user_id] || '0') || 0;
            ...
            const overtimeTotal = overtimeEntries.reduce(
                (s, e) => s + calculateOvertimeAmount(basicSalary, e.hours, e.type), 0
            );
            const loanAmount = empExtras?.loan_amount ?? 0;

            return calculatePayrollRow({
                emp, agreedNetPay, workingDays, nonTaxableTransport, taxableTransport,
                penaltyHours, overtimeTotal, loanAmount, expectedHours,
            });
        });
    }, [employees, localEdits, config, manualPenalties, holidays, exemptions, extras]);

    const totals = useMemo(() => ({
        agreedNetPay: rows.reduce((s, r) => s + r.agreedNetPay, 0),
        basicSalary: rows.reduce((s, r) => s + r.basicSalary, 0),
        workingDays: rows.reduce((s, r) => s + r.workingDays, 0),
        salaryForWorkingDays: rows.reduce((s, r) => s + r.salaryForWorkingDays, 0),
        nonTaxableTransport: rows.reduce((s, r) => s + r.nonTaxableTransport, 0),
        taxableTransport: rows.reduce((s, r) => s + r.taxableTransport, 0),
        overtimeTotal: rows.reduce((s, r) => s + r.overtimeTotal, 0),
        hourPenaltyAmount: rows.reduce((s, r) => s + r.hourPenaltyAmount, 0),
    ...
    const setAgreedNetPay = (id: string, val: string) =>
        setConfig((prev) => ({ ...prev, agreedNetPays: { ...prev.agreedNetPays, [id]: val } }));
    const setWorkingDays = (id: string, val: string) =>
        setConfig((prev) => ({ ...prev, workingDays: { ...prev.workingDays, [id]: val } }));
    const setTransport = (id: string, val: string) =>
        setConfig((prev) => ({ ...prev, transports: { ...prev.transports, [id]: val } }));
    ...
    const overtimeBasicSalary = useMemo(() => {
        if (!overtimeTarget) return 0;
        const agreed = parseFloat(config.agreedNetPays[overtimeTarget] || '0') || 0;
        const transport = parseFloat(config.transports[overtimeTarget] ?? config.defaultTransport) || 0;
        return agreed > 0 ? solveBasicFromAgreedNet(agreed, transport) : 0;
    }, [overtimeTarget, config.agreedNetPays, config.transports, config.defaultTransport]);
    const overtimeEntries = overtimeTarget ? (extras[overtimeTarget]?.overtime_entries ?? []) : [];

    const TABLE_HEADERS = [
        'Employee', 'Ignore Penalty', 'Agreed Net Pay', 'Basic Salary', 'Working Days', 'Salary (WD)',
        'Transport', 'Penalty Hrs', 'Hour Penalty', 'Overtime', 'Loan',
        'Gross Pay', 'Taxable Income', 'Income Tax', 'Pension 7%', 'Pension 11%', 'Total Deduct.', 'Net Pay',
    ];
        ...
        <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-end gap-4 px-5 py-4 rounded-2xl border"
                ...
                <div className="flex items-start gap-2 px-3 py-2 rounded-xl text-xs flex-1"
                    ...
                    <span>Enter each employee&apos;s <strong>agreed net pay</strong> (take-home at full attendance, no OT, no penalty). Basic salary, tax, and pension are calculated from that. OT = basic÷30÷8 × hours × multiplier.</span>
                </div>
                    ...
                    <table className="w-full border-collapse" style={{ minWidth: 1800 }}>
                        ...
                        <tbody>
                            {rows.map((row, idx) => {
                                ...
                                const hasSalary = row.agreedNetPay > 0;
                                    ...
                                    <tr key={row.zkt_user_id} style={{ background: idx % 2 === 0 ? 'var(--admin-card)' : 'var(--admin-bg)' }}>
                                        ...
                                        <td style={COL}>
                                            <button onClick={() => toggleIgnorePenalty(emp.zkt_user_id)}
                                                style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 6, fontSize: 10, fontWeight: 700, cursor: 'pointer', border: 'none', background: config.ignoreTimePenalty[emp.zkt_user_id] ? 'rgba(22,163,74,0.15)' : 'var(--admin-bg)', color: config.ignoreTimePenalty[emp.zkt_user_id] ? 'var(--admin-success, #16A34A)' : 'var(--admin-text-secondary)', outline: `1px solid ${config.ignoreTimePenalty[emp.zkt_user_id] ? 'rgba(22,163,74,0.3)' : 'var(--admin-border)'}` }}>
                                                <span style={{ width: 28, height: 16, borderRadius: 8, display: 'inline-flex', alignItems: 'center', background: config.ignoreTimePenalty[emp.zkt_user_id] ? 'var(--admin-success, #16A34A)' : 'var(--admin-border)', padding: '0 2px' }}>
                                        ...
                                        <td style={COL}>
                                            <input type="number" min="0" placeholder="0"
                                                value={config.agreedNetPays[emp.zkt_user_id] ?? ''}
                                                onChange={(e) => setAgreedNetPay(emp.zkt_user_id, e.target.value)}
                                        ...
                                        <td style={{ ...COL, fontWeight: 600, color: 'var(--admin-text-secondary)' }}>
                                            {hasSalary ? formatSalary(row.basicSalary) : '—'}
                                        </td>

                                        <td style={COL}>
                                            <input type="number" min="0" max="30" step="1"
                                                value={config.workingDays[emp.zkt_user_id] ?? DEFAULT_WORKING_DAYS}
                                                onChange={(e) => setWorkingDays(emp.zkt_user_id, e.target.value)}
                                                className="rounded-lg px-2 text-xs font-medium outline-none"
                                                style={{ height: 30, width: 60, background: 'var(--admin-bg)', border: '1px solid var(--admin-border)', color: 'var(--admin-text)' }} />
                                        </td>

                                        <td style={{ ...COL, color: 'var(--admin-text)' }}>
                                            {hasSalary ? formatSalary(row.salaryForWorkingDays) : '—'}
                                        </td>
                                        ...
                                        <td style={COL}>
                                            {hasSalary && row.hourPenaltyAmount < 0 ? (
                                                <span style={{ color: 'var(--admin-danger, #DC2626)', fontWeight: 600 }}>
                                                    {formatSalary(row.hourPenaltyAmount)}
                                                </span>
                                        ...
                                        <td style={COL}>
                                            <div className="flex items-center gap-1">
                                                ...
                                                <button onClick={() => setOvertimeTarget(emp.zkt_user_id)}
                                        ...
                                        <td style={{ ...COL, fontWeight: 600, color: 'var(--admin-text)' }}>{hasSalary ? formatSalary(row.grossPay) : '—'}</td>
                                        <td style={{ ...COL, color: 'var(--admin-text)' }}>{hasSalary ? formatSalary(row.taxableIncome) : '—'}</td>
                                        <td style={{ ...COL, color: 'var(--admin-danger, #DC2626)', fontWeight: 600 }}>{hasSalary ? formatSalary(row.incomeTax) : '—'}</td>
                                        <td style={{ ...COL, color: 'var(--admin-danger, #DC2626)' }}>{hasSalary ? formatSalary(row.pensionEmployee) : '—'}</td>
                                        <td style={{ ...COL, color: 'var(--admin-text-secondary)' }}>{hasSalary ? formatSalary(row.pensionEmployer) : '—'}</td>
                                        <td style={{ ...COL, color: 'var(--admin-danger, #DC2626)', fontWeight: 700 }}>{hasSalary ? formatSalary(row.totalDeduction) : '—'}</td>
                                        <td style={{ ...COL, fontWeight: 700, color: 'var(--admin-success, #16A34A)', fontSize: 13 }}>{hasSalary ? formatSalary(row.netPay) : '—'}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
                        <tfoot>
                            <tr style={{ background: 'var(--admin-bg)', borderTop: '2px solid var(--admin-border)' }}>
                                ...
                                <td style={COL} />
                                <td style={{ ...COL, fontWeight: 700 }}>{formatSalary(totals.agreedNetPay)}</td>
                                <td style={{ ...COL, fontWeight: 700 }}>{formatSalary(totals.basicSalary)}</td>
                                <td style={{ ...COL, fontWeight: 700 }}>{totals.workingDays}</td>
                                <td style={{ ...COL, fontWeight: 700 }}>{formatSalary(totals.salaryForWorkingDays)}</td>
                                <td style={{ ...COL, fontWeight: 700 }}>{formatSalary(totals.nonTaxableTransport)}</td>
                                <td colSpan={1} />
                                <td style={{ ...COL, fontWeight: 700, color: 'var(--admin-danger, #DC2626)' }}>{totals.hourPenaltyAmount < 0 ? formatSalary(totals.hourPenaltyAmount) : '—'}</td>
                                ...
                                <td style={{ ...COL, fontWeight: 700 }}>{formatSalary(totals.grossPay)}</td>
                                <td style={{ ...COL, fontWeight: 700 }}>{formatSalary(totals.taxableIncome)}</td>
                                ...
                                <td style={{ ...COL, fontWeight: 700, color: 'var(--admin-success, #16A34A)', fontSize: 13 }}>{formatSalary(totals.netPay)}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            {overtimeTarget && overtimeEmployee && (
                <OvertimeModal
                    ...
                    basicSalary={overtimeBasicSalary}
            ...
            {showExportModal && (
                <PayrollExportModal rows={rows} period={exportPeriodLabel} onClose={() => setShowExportModal(false)} />
            )}
        </div>
    );
}
