...
const OrderCard = ({ order, onViewDetails, onMenuClick }: OrderCardProps) => {
  ...
  const totalPrice = parseFloat(order.full_payment || 0);
  const advancePayment = parseFloat(order.advance_payment || 0);
  const percentPaid = totalPrice > 0 ? (advancePayment / totalPrice) * 100 : 0;

  // Logic to show only first 5 orders, then +X more
  ...
  return (
    ...
    >
      ...
      {/* --- BODY: Order Codes List (The requested feature) --- */}
      ...
      {/* --- FOOTER: Financials --- */}
      <div className="p-4 bg-gray-50/50 dark:bg-zinc-900/50 mt-auto border-t border-gray-100 dark:border-zinc-800">
        <div className="flex justify-between items-end mb-2">
          <div>
            <span className="text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">
              Total Amount
            </span>
            <div className="text-lg font-bold text-gray-900 dark:text-white leading-none mt-0.5">
