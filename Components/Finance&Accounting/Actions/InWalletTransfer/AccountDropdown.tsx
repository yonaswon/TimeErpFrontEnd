...
interface AccountDropdownProps {
  ...
  disabled?: boolean
  required?: boolean
  invoice?: boolean // Add invoice prop to determine account type
}

export const AccountDropdown = ({ value, onChange, disabled = false, required = false, invoice = false }: AccountDropdownProps) => {
  ...
  }, [isOpen, invoice]) // Add invoice to dependencies
  ...
  return (
    <div ref={dropdownRef} className="relative w-full mt-2"> {/* Added margin-top here */}
      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
        To Account {required && <span className="text-red-500">*</span>}
      </label>
          ...
          {loading ? (
            ...
            accounts.map((account) => (
              ...
              >
                ...
                {value === account.id && <Check size={16} className="text-blue-500 flex-shrink-0 ml-2" />}
              </button>
            ))
          )}
        </div>
