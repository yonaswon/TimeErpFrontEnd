...
type TaskType = 'cutting' | 'assembly' | 'delivery';
...
export const LatestAssigned = () => {
  ...
  const tabs = [
    ...
    { id: 'delivery' as TaskType, label: 'Delivery & Installation', icon: Truck },
  ];
...
const CuttingTasks = ({ files, loading, error, onEdit, onRefresh }: CuttingTasksProps) => {
...
const CuttingTaskCard = ({ file, onEdit }: CuttingTaskCardProps) => {
  ...
  const getStatusColor = (status: string) => {
    ...
    return statusConfig[status as keyof typeof statusConfig] || statusConfig['NOT-ASSIGNED'];
  };
            ...
            <div>
              ...
              <div className="space-y-1">
                {file.orders.map((order) => (
                  <div key={order.order_code} className="flex items-center justify-between text-sm">
